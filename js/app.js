...
Enemy.prototype.randomize = function() {
    ...
    } while (app.collides(this, app.allEnemies));
};
...
Enemy.prototype.update = function(dt) {
    ...
    if (app.collides(this, [app.player])) { // if enemy collides with player
        app.player.score = Math.max(0, app.player.score - 100); //   -100 points    
        app.player.startLocation(); //   place player at starting location
    }
    if (this.x > app.CANVAS_WIDTH) { // if enemy moves off right of screen
        this.randomize(); //   generate a random left of screen location
    }
};
...
Player.prototype.update = function(key) {
    if (this.pause && key == 'space') { // new game
        ...
        if (app.allEnemies.length > 5) { // reduce enemies to 5
            app.allEnemies.splice(5, app.allEnemies.length - 5);
        }
    ...
    } else if (key == 'up' && this.y > app.ROW_Y[1]) { // move up
        this.y -= app.ROW_HEIGHT; // decrement y
        if (this.y == app.ROW_Y[1]) { // if crossed road
        ...
        this.y += app.ROW_HEIGHT;
    } // move down
    else if (key == 'left' && this.x > app.COL_X[1]) {
        this.x -= app.COL_WIDTH;
    } // move left
...
Player.prototype.render = function() {
    ...
    ctx.clearRect(0, 0, app.CANVAS_WIDTH, 50); // display score & time
